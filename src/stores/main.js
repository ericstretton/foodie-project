...
// #1 Post to register new users
        createUser(username, firstName, lastName, email, password, pictureUrl){
            axios.request({
                ...
                cookies.set('clientUserToken', response.data.token);
                console.log(cookies.get('clientUserToken'));
                router.push('/explore');
            }).catch((error) =>{
...
//  #4 View Existing Client User Information on Client Profile Page
        viewUserInfo(clientId, createdAt, email, firstName, lastName, pictureUrl, username){
            axios.request({
                headers:{
                    "x-api-key" : process.env.VUE_APP_API_KEY,
                    "token" : cookies.get('clientUserToken'),
                },
                url: process.env.VUE_APP_API_URL+"client",
                method : "GET",
                data: {
...
// #2 Restaurant Client Login 
        restaurantLogin(email, password){
            axios.request({
                ...
                cookies.set('rsetaurantToken', response.data.token);
                console.log(cookies.get('restaurantToken'));
                router.push('/restaurant-profile')
            }).catch((error) =>{
                console.log(error);
...
// #3 View Restaurant Information ALL users for unique restaurant Id
        getRestaurantInfo(){
            axios.request({
                ...
                this.restaurantId = response.data[0].restaurantId
                console.log(response);
            }).catch((error)=>{
                console.log(error);
            })
...
// #1 Client Creating Orders
    userCreateOrder(restaurantId, items){
        axios.request({
            headers: {
                "x-api-key" : process.env.VUE_APP_API_KEY,
                "token" : cookies.get('clientUserToken')
            ...
            console.log(response +"Order Created");
        }).catch((error) => {
            console.log(error);
        })
    },

    // #2(a) Get request for Client to view order history
    clientViewOrders(){
        axios.request({
            headers: {
                "x-api-key" : process.env.VUE_APP_API_KEY,
    ...
    // #2(b) Get Request for Restaurant to view all current orders
